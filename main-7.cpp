...
struct net
{
    ...
    double
    x(const double i) const { return a1 + h1 * i; }

    double
    y(const double j) const { return a2 + h2 * j; }
};
...
public:
    ...
    {
        matrix res{};
        for (size_t i = 0; i < M; ++i) { for (size_t j = 0; j < N; ++j) { base[i][j] = static_cast<T>(0); } }
        return res;
    }
...
#pragma omp parallel for default(none) shared(res)
    ...
    T &
    operator()(size_t i, size_t j) { return base[i][j]; }

    const T &
    operator()(size_t i, size_t j) const { return base[i][j]; }
...
#pragma omp parallel for default(none) shared(res, left, right)
        for (size_t i = 0; i < M; ++i) { for (size_t j = 0; j < N; ++j) { for (size_t k = 0; k < K; ++k) { res(i, j) += left(i, k) * right(k, j); } } }
        return res;
    }

    matrix &
    operator+=(const matrix &other) { return *this = *this + other; }

    matrix &
    operator-=(const matrix &other) { return *this = *this - other; }

    matrix &
    operator*=(T k) { return *this = *this * k; }

    template<size_t K>
    matrix<T, M, K> &
    operator*=(const matrix<T, N, K> &other) { return *this = *this * other; }
...
public:
    ...
    T &
    operator()(size_t i) { return matrix<T, M, 1>::operator()(i, 1); }

    const T &
    operator()(size_t i) const { return matrix<T, M, 1>::operator()(i, 1); }

    template<size_t N>
    friend vector
    operator*(const matrix<T, M, N> &left, const vector<T, N> &right) { return left * static_cast<matrix<T, N, 1>>(right); }
...
#pragma omp parallel for default(none) shared(res, w, h)
    for (size_t i = 1; i < M - 1; ++i) { for (size_t j = 1; j < N - 1; ++j) { res(i, j) = (w(i + 1, j) - w(i, j)) / h; } }
    return res;
}
...
left_x_derivative(const matrix<double, M, N> &w, double h)
{
    matrix<double, M, N> res = matrix<double, M, N>::zero();
#pragma omp parallel for default(none) shared(res, w, h)
    for (size_t i = 1; i < M - 1; ++i) { for (size_t j = 1; j < N - 1; ++j) { res(i, j) = (w(i, j) - w(i - 1, j)) / h; } }
    return res;
}
...
right_y_derivative(const matrix<double, M, N> &w, double h)
{
    matrix<double, M, N> res = matrix<double, M, N>::zero();
#pragma omp parallel for default(none) shared(res, w, h)
    for (size_t i = 1; i < M - 1; ++i) { for (size_t j = 1; j < N - 1; ++j) { res(i, j) = (w(i, j + 1) - w(i, j)) / h; } }
    return res;
}
...
left_y_derivative(const matrix<double, M, N> &w, double h)
{
    matrix<double, M, N> res = matrix<double, M, N>::zero();
#pragma omp parallel for default(none) shared(res, w, h)
    for (size_t i = 1; i < M - 1; ++i) { for (size_t j = 1; j < N - 1; ++j) { res(i, j) = (w(i, j) - w(i, j - 1)) / h; } }
    return res;
}
...
main(int argc, char **argv)
{
    ...
    const auto solution = solve<m, n>([](const double x, const double y) { return x * x + 4 * y * y < 1; }, -1, 1, -0.5, 0.5);
    MPI_Finalize();
    if (rank == 0) {
